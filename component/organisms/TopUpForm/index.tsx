...
import { BanksType, NominalType, PaymentType } from '../../../services/data.types';
...
export default function TopUpForm(props: TopUpFormType) {
  ...
  const onNominalItemChange = (data: NominalType) => {
    setNominalItem(data);
  };
  const onPaymentItemChange = (payment: PaymentType, bank: BanksType) => {
    const data = {
      payment,
      bank,
    };
    ...
    <>
      ...
      <div className="pt-md-50 pb-md-50 pt-30 pb-20">
        ...
        <div className="row justify-content-between">
          {nominals.map((nominal: NominalType) => (
            <NominalItem
              key={nominal._id}
              _id={nominal._id}
        ...
        <fieldset id="paymentMethod">
          <div className="row justify-content-between">
            {payments.map((payment: PaymentType) => (
              payment.banks.map((bank: BanksType) => (
                <PaymentItem
                  key={bank._id}
                  bankID={bank._id}
                  type={payment.type}
